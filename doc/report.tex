...
\assistant{Dr. Tilmann Zaeschke \\Christoph Zimmerli}
...
The experiments were executed to measure the insertion response time and throughput for distributed clusters containing different numbers of server hosts. The machines used were provided by the Google Computer Engine\footnote{https://cloud.google.com/compute/} cloud platform. Each server process was ran on a virtual instance containing 2 virtual CPU's and 7.5 GB of RAM memory\footnote{The code used by Google Compute engine for this type of virtual machine is \texttt{n1-standard-2 }}. The physical hardware is not disclosed by the Google Cloud platform, however the virtual machine emulated the Intel Core i7-3800 processor family, with each of the 2 CPU cores having a 2.6GHz frequency. Client requests were generated from virtual instances with the same hardware resources as the server machines. 
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.5\linewidth}
        ...
        \caption{KNN with k = 50 query throughput}
    \end{subfigure}%
    \begin{subfigure}[b]{0.5\linewidth}
        \includegraphics[scale=0.5]{images/plots/knn-distributed-rt}
        \caption{KNN with k = 50 response time}
    \end{subfigure}
...
The concurrent experiments have been run on a Google Compute Engine VM\footnote{The code of the virtual machine on the Google Compute Engine platform is \texttt{n1-standard-16}.} instance with 16 vCPU's and 60GB of RAM. Similar to the machines used for the distribution computation, the actual hardware used by the VM is not disclosed. However, the emulatd hardware family is Intel Xeon E5-1600, and each virtual CPU has a frequency of 2.5 GHz. 
...
The experiments were performed for multi-dimensional entries with an increasing number of dimensions. Figure \ref{fig:concurrentd} shows the response time and throuput for the inserion operation, with the number of threads varying from 1 to 16. The figure shows the results of the same experiment on PH-tree having dimensions 1, 2, 3, 6 and 10.  
...
\begin{figure}[h]
    \centering
    \begin{subfigure}[b]{0.5\linewidth}
        \includegraphics[scale=0.5]{images/plots/read1d-tp}
        \caption{1D PH-tree throughput}
    \end{subfigure}%
    \begin{subfigure}[b]{0.5\linewidth}
        \includegraphics[scale=0.5]{images/plots/read1d-rt}
        \caption{1D PH-tree response time}
    \end{subfigure}
    \begin{subfigure}[b]{0.5\linewidth}
        \includegraphics[scale=0.5]{images/plots/read2d-tp}
        \caption{2D PH-tree throughput}
    \end{subfigure}%
    \begin{subfigure}[b]{0.5\linewidth}
        \includegraphics[scale=0.5]{images/plots/read2d-rt}
        \caption{2D PH-tree response time}
    \end{subfigure}
    \begin{subfigure}[b]{0.5\linewidth}
...
Finally, the last type of tests evaluate the scalability of the PH-tree read operations with respect to the number of concurrent reader threads. In this test, the PH-tree is loaded with 5 million entries and then all entries inserted in the tree are retrieved with the \textit{get()} point query. The entries are split into non-overlapping partitions and each partition is assigned to a reader thread. Each thread then retrieves all of the keys in its partition. As previously mentioned, the concurrent access stratgies did not modify the read operatons and all implemented concurrent strategies perform the read operations in the same manner. 

These tests have been run on an 16 CPU machine, with 60GB of RAM memory. The number of reader threads is varied from 1 to 16, to reduce the effects of thread context switching. The test was repeated for PH-trees of dimensions 1, 2, 3, 6 and 10, to observe the effect of the higher dimensional data in the overall performance of the PH-tree. Figure \ref{fig:concurrent-read} presents the evolution of the throughput and response time for the described setups.

The reslts shown that an incresed number of reader threads increases the overall throughput of the PH-tree. The increase in throughput is almost linear for up to 6 reader threads, after which each additional reader thread adds a smaller increase to the throughput. The throughput stabilizes after the number of readers goes over 10.
\section{Result}
\label{sec:evaluation-results}

The distributed experiments showed that the distributed PH-tree scales with an increasing number of server machines. In all experiments of this type, the addition of server machines to the cluster increased the overall throughput of the system and decreased the response time for each individual request. For a fixed size workload, the throughput increase and response time decrease is slightly smaller with each additional server. This effect is due to the fact that test environment contains a limited number of clients, which wait for a response from the server before creating the next request. 

The concurreny experiments compared the performance of the three implemented concurrency strategies: \textit{Copy-on-Write}, \textit{Hand-over-Hand locking} and \textit{Optimistic locking}.\textit{Optimistic locking} performed better for the write operations, having a higher throughput and a lower response time than both other strateiges. \textit{Hand-over-Hand locking} performs better than \textit{Copy-on-Write} for smaller dimensions and up to 3 or writer threads, depending on the operation. The low performance of \textit{Hand-over-Hand locking} higher number of writers is caused by the incresed lock contention that appears in these settings. Generally, \textit{Optimistic locking} provides the best performance for both read and write operations.
...
The distribution architecture partitions the multi-dim"Cele mai importante două zile din viața ta sunt ziua în care te-ai născut și cea în care afli de ce."ensional space in a manner that allows fast point operation and efficient range and nearest neighbour queries. The proposed balancing algorithm ensures that all hosts will eventually hold a comparable number of entries, even in the case of very skewed input distributions. The presented distribution solution can be easily adapted for other types of index keys, as long as the key domain can be mapped to 1 dimensional line, allowing the constructing of efficient and scalable range-queryable, distributed key-values stores.
